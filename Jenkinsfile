...
pipeline {
    ...
    environment {
        ...
        /* groovylint-disable-next-line LineLength */
        TARGETGROUP_UTOPIA_ADMIN_DEV_ARN = "${sh(script:'echo $AWS_SECRET_TARGET_GROUPS | jq -r \'. | .TARGETGROUP_UTOPIA_ADMIN_DEV_ARN \'', returnStdout: true)}"
        /* groovylint-disable-next-line LineLength */
        TARGETGROUP_UTOPIA_ADMIN_PROD_ARN = "${sh(script:'echo $AWS_SECRET_TARGET_GROUPS | jq -r \'. | .TARGETGROUP_UTOPIA_ADMIN_PROD_ARN \'', returnStdout: true)}"
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                echo "$TARGETGROUP_UTOPIA_ADMIN_DEV_ARN"
                echo "$TARGETGROUP_UTOPIA_ADMIN_PROD_ARN"
